...
const cardVariants = {
  ...
  visible: {
    ...
    y: 0,
    scale: 1,
    transition: { duration: 0.8, type: "spring" as const, stiffness: 120, damping: 12 },
  },
};
...
export default function GoalsAndObjectives() {
  return (
    <section className="bg-sky-200 py-14 px-4 sm:px-6 md:px-8">
      <div className="max-w-7xl mx-auto">
        <motion.h2
          ...
          viewport={{ once: true }}
          className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-800 mb-10 text-center relative font-serif"
        >
          <span className="opacity-10 absolute -z-10 text-6xl sm:text-8xl top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 select-none">
            SERVICE
          </span>
        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
          {objectives.map((obj, index) => (
            <motion.div
              key={index}
              className="p-5 sm:p-6 border border-zinc-100 rounded-lg shadow-sm bg-slate-600 text-white font-serif"
              ...
              viewport={{ once: true }}
            >
              <h3 className="text-lg sm:text-xl font-semibold mb-2">{obj.title}</h3>
